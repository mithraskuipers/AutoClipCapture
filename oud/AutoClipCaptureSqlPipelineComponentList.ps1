...
=====================================================================
 ...
 match the pipeline's. "End of list" is detected two ways:

   1. Explicit marker (checked first, on the RAW captured text before
      any row-trimming - the marker might live in a row that
      SkipRowsEnd would otherwise cut off): if
      ComponentList.EndOfListText (default "Bottom of List") appears
      anywhere on the page, that page is known to be the last one.
      It's still saved like any other page (it may still contain real
      component rows above the marker), but no next-page key is sent
      afterward - there's nothing to gain from turning a page that
      already told us it's the end.

   2. Fallback similarity check, for screens that don't show any such
      marker: the same Get-TextSimilarity comparison the classic
      Toggle relay uses to detect a stuck capture
      (ComponentList.DupDetectThreshold - default 0.995) against the
      previous page. Unlike the classic relay, this stops
      automatically instead of asking, since running off the end of a
      bounded list is the expected, successful outcome rather than
...
function Invoke-PipelineListTick {
    ...
    switch ($global:CR_PipelineState) {
        ...
        'ListCapture_Wait' {
            ...
            # Checked on the RAW text, before trimming - the marker
            # could well sit inside the very last rows that
            # SkipRowsEnd is about to cut off.
            $sawEndMarker = Test-RelayTextContains -Text $text -Needle $listCfg.EndOfListText

            if ($sawEndMarker) {
                # This page told us outright that it's the last one -
                # still worth saving (it can still hold real component
                # rows above the marker), but there's nothing to gain
                # from pressing the next-page key, so skip straight to
                # done instead of turning one more page just to have
                # the similarity check confirm what we already know.
                ...
                Write-Host "[AutoClipCapture] [$($pipeline.Name)] Component list: page $($global:CR_PipelineListPageIdx) captured - found '$($listCfg.EndOfListText)', this is the last page." -ForegroundColor Cyan
